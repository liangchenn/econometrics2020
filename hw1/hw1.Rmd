...
taking `beta` = `{beta1, beta2}` as arguments and ouputing a numerical vector of log-likelihood.  

(Adding an option `type` to choose the output to be either vector or the sum of log-likelihood value.)  
...
here we define a function to expand the grid of parameters and report the maximum likelihood realisation row.
```{r}
...
Here we use `BHHH` and `Nelder Mead` methods provided by `maxLik` package.  
...
first, we rewirte log likelihood function to meet the requirement of `maxLik` function.
...
Define a function to show the final result, including estimated beta and its standard deviation.
